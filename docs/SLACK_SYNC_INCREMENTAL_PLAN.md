...
また、清水光一さんの自己紹介メッセージは `data/slack-messages.jsonl` とメッセージ検索インデックスには存在していたが、`data/employees.json` には社員として登録されていなかった。そのため検索インデックス上では `slack-user:U0B0XCRK6KW` 扱いになり、社員候補として安定して回答に出にくい状態だった。
...
- 新規/更新メッセージを検索インデックスとGitHub Pagesのメッセージ一覧へ反映する。
...
- 現在の `data/slack-messages.jsonl` を使ったドライランで清水光一さんが社員登録対象になることを確認
- 手動workflow実行で `Sync Slack Activity` と `Build Knowledge Graph` が短時間で通過することを確認

### PR 2: Slack取得の差分化を強める
...
- `data/employees.json` に清水光一さんが `slack_id: U0B0XCRK6KW` で登録されている。
- `data/message-search-index.json` と `data/message-search-index.embedded.json` の清水光一さんのメッセージ検索ユニットが `slack-user:*` ではなく社員として扱われる。
- `ゲーセン好き` で清水光一さんの自己紹介メッセージが候補に入る。
- GitHub Pagesの `docs/slack-export` に最新メッセージが反映される。
...
1. この計画書をレビューする。
2. PR 1として通常同期の軽量モードを実装する。
3. PR 1マージ後に軽量同期を手動実行し、清水光一さんを含む未登録社員と最新メッセージを反映する。
4. 実行ログと検索結果を見て、PR 2以降の差分化範囲を決める。
